import dimod
import matplotlib.pyplot as plt
import mlflow
import numpy as np
import pandas as pd
import seaborn as sns
from dwave.system import LeapHybridSampler
from tqdm import tqdm

# ExcelファイルからEEGデータの読み込み
original_eeg_data = pd.read_excel("/Users/yamayuu/BoltzmannMachine_pre/EEG_all.xlsx")

# 初期設定
bin_size = 10
regions = ["F3", "F4", "Fz", "C3", "C4", "P3", "P4", "A1", "A2"]
sampling_rate = 250

# データの前処理
binarized_data = {}
for region in regions:
    c_data = original_eeg_data[region].to_numpy()
    smoothed_c_data = np.convolve(c_data, np.ones(bin_size) / bin_size, mode="same")
    binarized_c_data = np.where(c_data > smoothed_c_data, 1, -1)
    binarized_data[region] = binarized_c_data

n = 12  # 任意の次元数に変更可能
combined_data = []

# 各領域について、隣接するn個のbinを組み合わせたベクトルを作成
for i in range(len(binarized_data[regions[0]]) - n + 1):
    combined_bin = []
    for j in range(n):
        combined_bin.extend([binarized_data[region][i + j] for region in regions])
    combined_data.append(np.array(combined_bin))

combined_data = np.array(combined_data)


size = 108  # 18次元のベクトルを扱う


class QuantumBoltzmannMachine:
    def __init__(self, size, token):
        self.size = size
        self.W = np.zeros((size, size))
        self.b = np.zeros(size)
        self.token = token

    def to_ising(self):
        linear = {i: -self.b[i] for i in range(self.size)}
        quadratic = {
            (i, j): -self.W[i, j]
            for i in range(self.size)
            for j in range(i + 1, self.size)
        }
        return linear, quadratic

    def sample_dwave(self):
        linear, quadratic = self.to_ising()
        bqm = dimod.BinaryQuadraticModel(linear, quadratic, dimod.SPIN)

        # LeapHybridSamplerを使用してハイブリッドソルバーを指定
        sampler = LeapHybridSampler(token=self.token)
        sampleset = sampler.sample(bqm)

        return sampleset

    """
    # 学習率の自動調整機能を追加
    def autotune_learning_rate(self, data, learning_rates, epochs, lambda_l2, lambda_l1):
        best_lr = None
        best_energy = float('inf')
        for lr in learning_rates:
            self.W = np.zeros((self.size, self.size))
            self.b = np.zeros(self.size)
            energy = self.fit(data, shot=0, epochs=epochs, learning_rate=lr, lambda_l2=lambda_l2, lambda_l1=lambda_l1)[-1]
            if energy < best_energy:
                best_energy = energy
                best_lr = lr
        return best_lr
        """

    def fit(self, data, shot, epochs, learning_rate):
        energy_history = []
        for epoch in tqdm(
            range(epochs), desc=f"leap_Hybrid_Training Progress for Shot {shot + 1}"
        ):
            data_corr = np.mean([np.outer(d, d) for d in data], axis=0)
            sampleset = self.sample_dwave()  # num_samples 引数を削除
            model_samples = np.array(
                [list(sample.values()) for sample in sampleset.samples()]
            )
            model_corr = np.mean([np.outer(m, m) for m in model_samples], axis=0)
            self.W += learning_rate * (data_corr - model_corr)
            np.fill_diagonal(self.W, 0)
            self.b += learning_rate * (
                np.mean(data, axis=0) - np.mean(model_samples, axis=0)
            )
            energy = np.mean(
                [
                    -0.5 * np.dot(d.T, np.dot(self.W, d)) - np.dot(self.b.T, d)
                    for d in data
                ]
            )
            energy_history.append(energy)
        # 学習後のサンプリングされた解の分布を可視化
        plot_sample_distribution(model_samples, "Sample Distribution After Training")
        mlflow.log_figure(
            plt.gcf(), "leap_Hybrid_sample_distribution_after_training.png"
        )
        plt.close()
        # 重み行列の統計的特性の可視化
        plot_weight_distribution(self.W, f"Weight Distribution - Shot {shot + 1}")
        mlflow.log_figure(
            plt.gcf(), f"leap_Hybrid_weight_distribution_shot_{shot + 1}.png"
        )
        plt.close()

        return energy_history


# サンプリングされた解の分布の可視化
def plot_sample_distribution(samples, title):
    samples_df = pd.DataFrame(samples)
    sample_counts = samples_df.apply(pd.Series.value_counts, normalize=True).fillna(0)
    sample_counts.plot(kind="bar", stacked=True)
    plt.title(title)
    plt.xlabel("Node Index")
    plt.ylabel("Frequency")
    plt.legend(["-1", "1"], title="State")


def plot_weight_distribution(weights, title):
    plt.hist(weights.flatten(), bins=50)
    plt.title(title)
    plt.xlabel("Weight Value")
    plt.ylabel("Frequency")


# 相関ヒートマップと3Dプロットの関数定義
def plot_correlation_heatmap(data, title):
    correlation_matrix = np.corrcoef(data.T)
    plt.figure(figsize=(10, 8))
    sns.heatmap(correlation_matrix, cmap="coolwarm", annot=False)
    plt.title(title)
    mlflow.log_figure(
        plt.gcf(), f"correlation_heatmap_history_leap_Hybrid_72Dim_shot_{shot + 1}.png"
    )
    plt.close()
    # plt.show()


def plot_3d_correlation(data, title):
    fig = plt.figure(figsize=(10, 8))
    ax = fig.add_subplot(111, projection="3d")
    time_steps, nodes = data.shape
    X, Y = np.meshgrid(range(time_steps), range(nodes))
    Z = data.T
    ax.plot_surface(X, Y, Z, cmap="coolwarm")
    ax.set_title(title)
    mlflow.log_figure(
        plt.gcf(), f"3D_correlation_history_leap_Hybrid_72Dim_shot_{shot + 1}.png"
    )
    plt.close()
    # plt.show()


# MLflow実験の開始
with mlflow.start_run(run_name="leap_Hybrid_72Dimension Boltzmann Machine Training"):
    token = "DEV-f3f20a0429d3d20231b116e2131e7f35e53dea1d"
    qbm = QuantumBoltzmannMachine(size=size, token=token)

    num_shots = 1
    epochs = 50
    learning_rate = 0.05
    # num_samples = 20
    # lambda_l2 = 0.001
    # lambda_l1 = 0.001
    # 学習率の候補
    # learning_rates = [0.001, 0.05, 0.01, 0.5, 0.1]

    # パラメータの記録
    mlflow.log_param("num_shots", num_shots)
    mlflow.log_param("epochs", epochs)
    mlflow.log_param("learning_rate", learning_rate)
    # mlflow.log_param("num_samples", num_samples)
    # mlflow.log_param("lambda_l2", lambda_l2)
    # mlflow.log_param("lambda_l1", lambda_l1)
    mlflow.log_param("size", size)

    for shot in range(num_shots):
        # 学習率の自動調整
        # best_lr = qbm.autotune_learning_rate(combined_data, learning_rates, epochs, lambda_l2, lambda_l1)
        # mlflow.log_param(f"best_learning_rate_shot_{shot + 1}", best_lr)

        print(f"Starting training for leap_Hybrid_72Dim shot {shot + 1}")
        energy_history = qbm.fit(
            combined_data,
            shot=shot,
            epochs=epochs,
            learning_rate=sampling_rate,
            # lambda_l2=lambda_l2,
            # lambda_l1=lambda_l1,
        )

        # 相関ヒートマップと3Dプロットの生成
        plot_correlation_heatmap(
            combined_data, f"Correlation Heatmap - Shot {shot + 1}"
        )
        plot_3d_correlation(combined_data, f"3D Correlation Plot - Shot {shot + 1}")

        # エネルギー履歴のグラフ
        plt.figure(figsize=(10, 6))
        plt.plot(energy_history)
        plt.xlabel("Epoch")
        plt.ylabel("Energy")
        plt.title(f"Energy History - leap_Hybrid_72Dim Shot {shot + 1}")
        mlflow.log_figure(
            plt.gcf(), f"energy_history_leap_Hybrid_72Dim_shot_{shot + 1}.png"
        )
        plt.close()

        # 重みの可視化
        plt.figure(figsize=(10, 8))
        sns.heatmap(qbm.W, cmap="coolwarm", square=True)
        # plt.colorbar()
        plt.title("Weights Heatmap")
        mlflow.log_figure(plt.gcf(), f"weights_leap_Hybrid_72Dim_shot_{shot + 1}.png")
        plt.close()

        # バイアスの可視化
        plt.figure(figsize=(10, 6))
        plt.bar(range(qbm.size), qbm.b)
        plt.title("Biases")
        mlflow.log_figure(plt.gcf(), f"biases_leap_Hybrid_72Dim_shot_{shot + 1}.png")
        plt.close()
