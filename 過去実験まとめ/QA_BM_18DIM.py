...
original_eeg_data = pd.read_excel(
    "/Users/yamayuu/BoltzmannMachine_pre/EEG_all.xlsx"
)
...
for i in range(len(binarized_data[regions[0]]) - 1):
    combined_bin = np.array([binarized_data[region][i] for region in regions] +
...
class QuantumBoltzmannMachine(ExtendedBoltzmannMachine):
    def to_ising(self):
        # ボルツマンマシンの重みとバイアスをイジングモデルの形式に変換
    ...
    def sample_dwave(self, token, num_reads=100):
        # D-Wave Samplerを使用してサンプリングを行う
        linear, quadratic = self.to_ising()
        bqm = dimod.BinaryQuadraticModel(linear, quadratic, dimod.SPIN)
        dw_sampler = DWaveSampler(token=token)  # D-Wave Samplerのインスタンスを作成
        sampler = EmbeddingComposite(dw_sampler)
        sampleset = sampler.sample(bqm, num_reads=num_reads)
        return sampleset
    
    def fit(self, data, epochs, learning_rate, num_samples, lambda_l2, lambda_l1):
        # 学習処理の実装
        for epoch in range(epochs):
            ...
            # 重みとバイアスの更新
            self.W += learning_rate * (data_corr - model_corr)
            np.fill_diagonal(self.W, 0)  # 対角要素は0に設定
    ...
    def log_weights_and_biases(self, epoch):
        # 重みのグラフ
        plt.figure(figsize=(10, 8))
        sns.heatmap(self.W, annot=True, cmap="coolwarm")
        plt.title(f"Weights at Epoch {epoch}")
        mlflow.log_figure(plt.gcf(), f"weights_epoch_{epoch}.png")
        plt.close()

        # バイアスのグラフ
        plt.figure(figsize=(10, 8))
        plt.bar(range(self.size), self.b)
        plt.title(f"Biases at Epoch {epoch}")
        mlflow.log_figure(plt.gcf(), f"biases_epoch_{epoch}.png")
        plt.close()
...
num_reads = 1000  # D-Waveでのサンプリング回数
dwave_token = 'DEV-263b4b057d6b25ccfa4ec7421800fec1846b3423'  # ここにD-Waveのアクセストークンを入力



# QBMのインスタンス生成
qbm = QuantumBoltzmannMachine(size=18)

# 学習データの準備（例：combined_data）

# 学習処理の実行
qbm.fit(combined_data, epochs=epochs, learning_rate=learning_rate, num_samples=num_samples, lambda_l2=lambda_l2, lambda_l1=lambda_l1)
